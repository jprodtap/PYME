...
if "%currentValue%"=="" (
    echo La variable de entorno EJECUTAR_JAR no está definida. Estableciendo valor inicial a JAR.
    setx EJECUTAR_JAR "JAR" /M
    echo Variable de entorno EJECUTAR_JAR creada con el valor "A".
) else (
	...
	setx EJECUTAR_JAR "A" /M
         echo Variable de entorno EJECUTAR_JAR cambiada de "%currentValueOld%" a "A".
    )
)
