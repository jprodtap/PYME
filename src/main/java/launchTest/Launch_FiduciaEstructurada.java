...
public class Launch_FiduciaEstructurada extends BaseTestNG {
	// =======================================================================================================================
	// Page & Crontroller
	...
	ControllerFiducia controllerFiducia = null;
...
//		Evidence.siTomarla(Evidence.FORMATO_EXCEL); // TOMAR EVIDENCIAS POR ROW Y DEJA LO �LTIMO EN EL FORMATO INDICADO
		...
		// -----------------------------------------------------------------------------------------------------------------------
		// -----------------------------------------------------------------------------------------------------------------------
		Reporter.writeTitle("\n*** PRUEBAS PORTAL PYME FIDUCIA ESTRUCTURADA ***");
...
// *************************************************************************************************************************
		...
		// *************************************************************************************************************************
		/*
		 * OPCI�N SI DESEAS REALIZAR LA CONTRATACI�N, 1RE REALIZA EL FLUJO DE
		 * CONTRATACI�N DESDE MIDDLE Y PROCEDE A REALIZAR EL FLIJO EN FRONT
		 */
		if (contratacion.equals("SI")) {
			...
			if (msgError == null) {
				...
				this.pageServiciosMiddle = new PageServiciosMiddle(this.pageLogin);
				this.pageServiciosMiddle.validacionServicio();
		...
		if (contratacion.equals("NO")) {
			Reporter.reportEvent(Reporter.MIC_PASS, servicioSeleccionado);
			this.Front();
		}
	}
	...
	public void Front() throws Exception {
		...
		if (msgError == null) {
			...
			if (msgError == null) {
				// C�digo aqu�
				this.controllerFiducia = new ControllerFiducia(this.pageLogin);
				this.controllerFiducia.consultasFiducia();
				this.pageOrigen.terminarIteracion();
			} else {
